 ...
 *
 */
public class ModaI implements OperadorUnarioDeImagen {
	...
	public DocumentoDeImagen operar(DocumentoDeImagen entrada) {
		DocumentoDeImagen salida = new DocumentoDeImagen(entrada.getImagen(),entrada.getTitle()+"(Moda)",entrada.getPadre());
		salida.descomponer();
				...
				for(k=j-1;k<=j+1;k++)
				{
					for(l=i-1;l<=i+1;l++)
					{
						...
						if(n>=0)
							...
							actual.add(nuevo);
						//actual.addElement(new Integer());
					}
				}
				Collections.sort(actual);
				
				indexadorNuevo.set(actual.lastElement().getValor(),j,i);
			}			
		}
